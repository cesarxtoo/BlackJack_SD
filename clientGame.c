...
int main(int argc, char *argv[])
{
	...
	while (1)
	{ // cambiar a lo que le envie el server
		// ---- lee datos para enviar al server
		int fichas_apostadas = readBet();

		int bytes_enviados = send(socketfd, &fichas_apostadas, sizeof(int), 0);
		if (bytes_enviados < 0)
			showError("ERROR while writing to the socket");

		int opcion_elegida = readOption();

		bytes_enviados = send(socketfd, &opcion_elegida, sizeof(int), 0);
		if (bytes_enviados < 0)
			showError("ERROR while writing to the socket");

		// ---lee lo recibido del server
		while (1)
			;
	}
}
